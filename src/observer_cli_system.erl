...
%% support (e.g. BusyBox without CONFIG_DESKTOP on Nerves targets). CPU%
%% needs two time samples to derive a rate, so the gauge carrying the
%% reference sample is threaded explicitly through the caller's render loop;
%% there is no global/process-dictionary state here.
...
%% A BusyBox built without -o support rejects the command outright, and that
%% verdict cannot change while the view is open. Probing once at startup lets
%% the render loop skip os:cmd/1 entirely on those targets: forking a shell
%% costs ~900us, dwarfing the ~20us of /proc reads that replace it.
resolve_ps_cmd(Cmd) ->
    case ps_columns(os:cmd(Cmd)) of
        {ok, _Columns} -> Cmd;
        error -> no_ps
...
%% see observer_cli_lib:ps_output_fields/1 for how usage text gets here.
ps_columns(Output) ->
